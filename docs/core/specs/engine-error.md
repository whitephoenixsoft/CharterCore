...
Status: FROZEN (v2 – Participant Errors Added)  
...
The Engine must produce a structured, deterministic evaluation report for every command.
...
SUCCESS  
Command executed and mutated engine state.

REJECTED  
Command violated invariant or legitimacy rule.

BLOCKED  
Command rejected due to session block state or hygiene rule.

NO_OP  
Command was valid but resulted in no state change.
...
- PARTICIPANT_NOT_FOUND applies when:
  - Removing a non-existent participant
  - Recording vote from non-member
  - Referencing unknown participant in constraint

- CANNOT_REMOVE_LAST_PARTICIPANT applies when:
  - Removing a participant would result in empty participant set
  - Solo Mode invariant would be violated

Engine must prevent sessions from ever having zero participants.
...
- Engine emits non-enumerated error code
- Error semantics encoded only in diagnostics
...
- Engine mutates state on failure
- Block causes implicit closure
